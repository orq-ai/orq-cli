...
- Each message is a `<message index role>` element, whose index is its `--slice` position; nested `<reasoning>`, `<reasoning_summary>`, `<tool_call>`, `<error>`, and `<exception>` elements appear only when actual content exists. Message bodies are arbitrary recorded text, so this renderer's own tag names are escaped where they appear in content and cannot forge a turn. The opening `<thread>` tag reports the trace, span, dialect, model, duration, and token count, and reports a status only when the span failed. `--max-chars` bounds the text a single message renders, defaulting to 4000; the budget is spent across that message's blocks and shortens text inside elements only, so a truncated message stays well-formed.
...
- [ ] **Step 3: Implement span resolution**
  ...
  With an explicit span ID, hydrate only that span. Otherwise call get-trace, page through list-spans, build evaluator-descendant exclusions from `span_id`/`parent_span_id`, sort remaining unique detailed spans by `started_at` descending with stable tie-breaking, and hydrate candidates. Return the first normalizable conversation when it has no unavailable content; otherwise hydrate the rest and return the one with the most readable messages, preferring an intact conversation on a tie. Continue past hydration/API failures while retaining the first contextual operational error for use only if no candidate or fallback succeeds. Use `trace.leading_span_id` followed by `trace.root_span_id` as resilience fallbacks when listing is unavailable, empty, or has no supported span; apply evaluator-descendant exclusions to those fallbacks too, and never hydrate an ID twice when it was already listed or repeated as both fallback fields. Try every unique fallback so a valid root can recover from a stale leading ID; otherwise return the retained operational error, then a list error where applicable, and let only unsupported payloads fall through to the generic no-supported-conversation error. Treat type/name/operation values containing `evaluator` or the standalone token `eval` case-insensitively as evaluator spans. Return a clear error if no supported conversation exists.
