...
nor the config file reaches it: both state a standing default for every command
in a shell or on a machine, so one of them naming `json` would swap out the
render a person came to read, and `xml` and `markdown` are not values the rest
of the CLI accepts at all — a config file naming either fails every command,
this one included. Ask this command with `-o`.
...
| `ORQ_CLI_QUIET` | Set to `1` to run `install.sh` without its banner, info block, progress bar and next-steps epilogue. Its `!` warnings and errors still print. What `orq update` uses |
...
8.0.0 -> 8.5.2 (npm)`), and the child is kept quiet. npm's output is captured
and printed only if npm fails, so a successful update loses `changed 2 packages
in 2s` and a failed one still shows npm's own diagnosis. install.sh runs with
`ORQ_CLI_QUIET=1`, without the banner, the info block, the progress bar and the
next-steps epilogue it prints for a first install; its `!` warnings and errors
still come through as they happen.
