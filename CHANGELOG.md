...
- **Changed: `orq update` says what it is doing, and the child installer stops
  ...
  slow npm or download is no longer an unexplained wait. npm now runs at
  `--loglevel=error`, dropping lines like `changed 2 packages in 2s`, and
  install.sh is run in a new quiet mode that leaves out its banner, its
  installer/platform/version block, the download progress bar and the
  next-steps epilogue — all of which introduce a first install, not an
  in-place update. Warnings and errors from either child are untouched, as is
  `-o json`. The quiet mode is available to anyone running install.sh directly
  as `ORQ_CLI_QUIET=1`.
