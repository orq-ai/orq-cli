...
# this repository, verifies it against the published SHA256SUMS, and drops it at
...
if [ "${already_current:-0}" != "1" ]; then
  ...
  tmp_sums="$(mktemp -t orq-sums.XXXXXX)"
  cleanup() {
  ...
  # Releases published before SHA256SUMS existed have no sums file. A missing
  # file is a skip; only a mismatch is fatal.
  sha_cmd=""
  ...
  if [ -z "$sha_cmd" ]; then
    printf '! checksum not verified (no sha256sum or shasum available)\n'
  elif ! curl -fsSL -o "$tmp_sums" "$sums_url" 2>/dev/null || [ ! -s "$tmp_sums" ]; then
    printf '! checksum not verified (this release publishes no SHA256SUMS)\n'
  else
    expected="$(awk -v want="$asset" '$2 == want || $2 == "*"want {print $1; exit}' "$tmp_sums")"
    if [ -z "$expected" ]; then
      printf '! checksum not verified (%s missing from SHA256SUMS)\n' "$asset"
    else
      ...
      printf '✓ checksum verified (SHA256SUMS)\n'
    fi
  fi
