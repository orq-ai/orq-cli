...
// The label agents show for the orq provider in their own model pickers.
const (
	ProviderDisplayName          = "Orq AI Gateway"
	ProviderResponsesDisplayName = "Orq AI Gateway (Responses)"
)

// Caps for models the catalogue publishes no metadata for. Both err low
// deliberately: kimi sends max_tokens = max_output_size, so over-claiming makes
// the upstream reject the request outright, under-claiming only wastes capacity.
const (
