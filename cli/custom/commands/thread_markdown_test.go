...
func TestRenderThreadMarkdownCutsInsideTheFence(t *testing.T) {
	...
	var out bytes.Buffer
	if err := RenderThreadMarkdown(&out, thread, 20, 20); err != nil {
		t.Fatal(err)
	}
	rendered := out.String()
...
func TestRenderThreadMarkdownKeepsAnEmptyMessage(t *testing.T) {
	var out bytes.Buffer
	if err := RenderThreadMarkdown(&out, Thread{Messages: []ThreadMessage{{Index: 0, Role: "assistant"}}}, 0, 0); err != nil {
		t.Fatal(err)
	}
...
func TestRenderThreadMarkdownCapsRecordedCharacters(t *testing.T) {
	...
	var out bytes.Buffer
	if err := RenderThreadMarkdown(&out, thread, 4000, 4000); err != nil {
		t.Fatal(err)
	}
	rendered := out.String()
...
func TestRenderThreadMarkdownEscapesEverySourceField(t *testing.T) {
	...
	for index := range sourceType.NumField() {
		t.Run(sourceType.Field(index).Name, func(t *testing.T) {
			...
			var out bytes.Buffer
			if err := RenderThreadMarkdown(&out, Thread{Source: source.Interface().(ThreadSource)}, 0, 0); err != nil {
				t.Fatal(err)
			}
			rendered := out.String()
...
func TestThreadSourceFieldsListEverySourceField(t *testing.T) {
	...
	if err := RenderThread(&xml, Thread{Source: source}, 0, 0); err != nil {
		t.Fatal(err)
	}
	if err := RenderThreadMarkdown(&markdown, Thread{Source: source}, 0, 0); err != nil {
		t.Fatal(err)
	}
	for index := range sourceValue.NumField() {
...
func TestRenderThreadMarksAnUnencodableValue(t *testing.T) {
	...
	if err := RenderThreadMarkdown(&markdown, thread, 0, 0); err != nil {
		t.Fatal(err)
	}
	if err := RenderThread(&xml, thread, 0, 0); err != nil {
		t.Fatal(err)
	}
...
func TestRenderThreadMarkdownReportsAnErrorOnlySource(t *testing.T) {
	var out bytes.Buffer
	if err := RenderThreadMarkdown(&out, Thread{Source: ThreadSource{Error: "upstream timed out"}}, 0, 0); err != nil {
		t.Fatal(err)
	}
